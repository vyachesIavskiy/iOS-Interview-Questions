...
### 23. 🟢 Что такое `forEach`/`map`/`flatMap`/`compactMap`/`filter`/`reduce`? Какие еще примеры функционального программирования вы знаете в Swift?

`forEach`, `map`, `flatMap`, `compactMap`, `filter` и `reduce` - функции протокола `Collection` в Swift, которые дают возможность работать с коллекциями, используя функциональный подход.  
В данном случае:
