...
### 6. 🟢 `closure` - это `value type` или `reference type`? Почему? А `func`?
...
### 7. 🟢 Что такое `CoW` (Copy on Write)? Как и где он реализован в Swift?
...
### 10. 🟢 Назовите все уровни доступа и что они означают в Swift?
...
### 19. 🟢 Что такое `extension`? Что может быть в `extension`, а чего нет?
...
### 20. 🟢 Что такое `protocol`? Для чего они используются?
...
### 21. 🟢 Что такое `Any`? Что такое `AnyObject`? Для чего они используются?
...
### 22. 🟢 Назовите типы для работы с коллекциями, которые есть в Swift
...
### 23. 🟢 Что будет, если добавить в `Set` два объекта с одинаковым значением `hash`?
...
### 24. 🟢 Что такое `forEach`/`map`/`flatMap`/`compactMap`/`filter`/`reduce`? Какие еще примеры функционального программирования вы знаете в Swift?
...
### 25. 🟢 Что такое `weak` и `unowned`? Чем они отличаются?
...
### 27. 🟡 Что такое `lazy var`? Для чего она используется?
...
### 28. 🟡 Что такое `generics`? Для чего они используются?
