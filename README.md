...
Экземпляры value type хранятся в стеке, в то время, как reference type хранятся в куче.  
...
`closure` - это `reference type`. Если бы closure была бы value type, то при передачи ее в функцию, она бы копировалась, что привело бы к копированию всех объектов, которые попадают в ее список захвата и потерю контекста, где closure была определена.  
...
`CoW` - это механизм, при котором при присваивании одного экземпляра к другому, копирование объекта не происходит до тех пор, пока один из экземпляров не будет модифицирован. Данный механизм используется во всех `value type` типах данных в Swift.

<br/>

### 7. 🟢 Что такое `mutating`?

`mutating` - это ключевое слово в Swift, которое позволяет методу, который указан как `mutating`, менять `self` в `value type` типах данных. По умолчанию все методы `value type` типов данных `nonmutating`.
...
`private` - самый закрытый тип. Поля и методы типа с данным уровнем доступа видны только внутри объявления типа и расширениях, объявленных в данном файле.
...
Обе данных конструкции безопасно разворачивают `optional` и выполняют блок кода только в том случае, если значение `optional` не равно `nil`. Единственное различие в том, что `guard` обязан выйти из блока кода, где он был объявлен в своем `else` блоке.
